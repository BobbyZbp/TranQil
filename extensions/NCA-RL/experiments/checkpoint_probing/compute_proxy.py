"""Compute necessity scores on N sampled (s, a) from the offline buffer.

Loads a Stage-2 NCA-IQL checkpoint (Q + actor + necessity head) and
records, for each sampled state-action pair:
    N^off(s, a)   — Q-margin proxy        (paper §3.2)
    N_ψ(s, a)     — learned head ensemble (paper §4.1)
    Q_φ(s, a)     — sufficiency baseline  (paper §5.2 Q-rank baseline)

Output is a `.npz` consumed by `drop_test.py` (paired-rollout generation)
and `plots.py` (heatmap, decile, Spearman).
...
import d4rl  # noqa: F401  -- registers the D4RL envs with gym
import gym
import numpy as np
import pyrallis
import torch
...
if __name__ == "__main__":
    ...
    p.add_argument("--env", type=str, default="antmaze-medium-diverse-v2")
